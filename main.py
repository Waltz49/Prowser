...
def _frozen_subprocess_bootstrap() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--model-tasks-worker":
        ...
        from model_tasks_worker import main as model_tasks_main
...
# Local imports
from apple_events_handler import AppleEventsHandler, try_get_file_from_system
...
from theme_service import apply_theme, connect_system_theme_listener
from utils import (
    set_main_window,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Native Image Browser - Browse images with thumbnails and fullscreen view",
...
Examples:
  ...
  %(prog)s /path/to/images/ -p ~/.prowser-test  # Use custom profile directory
    ...
    parser.add_argument('-l', '-L', '--limit', type=int, metavar='N',
                        help='Limit display to N image files (0 means unlimited) *DEPRECATED*')
    ...
    if 'main_window' in globals() and main_window:
        try:
            ...
            if current_directory or current_file:
                # Save actual OS fullscreen state
                os_fullscreen_state = getattr(main_window, 'isFullScreen', lambda: False)()
                config.save_restore_state(current_file, current_directory, current_view_mode, os_fullscreen_state)
                # print(f"Saved restore state: file:{current_file}, directory:{current_directory}, view_mode:{current_view_mode}, os_fullscreen:{os_fullscreen_state}")
        except Exception:
...
def main():
    ...
    while True:
        ...
        if explicit_fullscreen_flag:
            ...
            # No explicit flags: check saved state for last known OS fullscreen preference
            config = get_config()
            restore_state = config.get_restore_state()
            if restore_state:
                last_os_fullscreen = restore_state.get('last_os_fullscreen')
                if last_os_fullscreen is not None:
                    # Use saved OS fullscreen state
                    args.no_fullscreen = not last_os_fullscreen
                else:
                    # No saved OS fullscreen state: default to fullscreen (first time)
                    args.no_fullscreen = False
            else:
                # No saved state at all: default to fullscreen (first time)
                args.no_fullscreen = False
    ...
    # Auto-adjust limit when image list is provided without --limit parameter
    if args.limit is None:
        # Get the current saved limit from config
        config = get_config()
        saved_settings = config.load_settings()
        saved_limit = saved_settings.get('max_images', 0)
        if saved_limit == 0:  # 0 means unlimited
            saved_limit = 99999
        
        # Use the larger of saved limit or number of images
        # Use the larger of saved limit or number of images
        args.limit = max(saved_limit, len(target_files))
    ...
    try:
        ...
        # Use saved settings as defaults, override with command line arguments if provided
        window_limit = args.limit if args.limit is not None else saved_settings.get('max_images', 0)
        if window_limit == 0:  # 0 means unlimited
            window_limit = 99999
        ...
        # Determine immediate_fullscreen: respect explicit flags, saved state, or default
        # immediate_fullscreen should be True only if we want OS fullscreen (not windowed)
        # If --no-fullscreen was explicitly passed, immediate_fullscreen must be False
        # Also, when opening a directory (not a specific file), never use immediate_fullscreen
        is_directory_only = directory and not target_files
        
        if is_directory_only:
            # When opening a directory, never use immediate fullscreen
            immediate_fullscreen = False
        elif getattr(args, '_explicit_no_fullscreen_flag', False):
            immediate_fullscreen = False
        elif getattr(args, '_explicit_fullscreen_flag', False):
            immediate_fullscreen = True
        else:
            # No explicit flags: use args.no_fullscreen (which was set from saved state or default)
            immediate_fullscreen = not args.no_fullscreen
        
        window = ImageBrowserWindow(
            limit=window_limit, 
            fullscreen=not args.no_fullscreen if not is_directory_only else False,  # Never fullscreen for directories
            target_file=None,  # We'll handle this via configuration
            immediate_fullscreen=immediate_fullscreen, 
        ...
        if args.limit is not None or args.filter is not None:
            if args.limit is not None:
                config.update_setting('max_images', args.limit)
            if args.filter is not None:
        ...
        if target_files:
            ...
            # Apply limit for specific files to enable windowing logic
            if args.limit is not None:
                configuration['limit'] = args.limit
            else:
                # If no limit specified and we're restoring thumbnail/slideshow mode, set a limit
                # to ensure windowed thumbnails are used instead of fullscreen view
                if hasattr(args, '_restore_state') and args._restore_state:
                    restore_state = args._restore_state
                    last_view_mode = restore_state.get('last_view_mode', 'thumbnail')
                    if last_view_mode in ['thumbnail', 'slideshow', 'slideshow2']:
                        configuration['limit'] = 300  # Use windowed thumbnails
            ...
            if window_filter:
                ...
                if not filter_matches_all_files:
                    ...
                    config.update_setting('limit', 300) # DGN persistance may not be necessary
            ...
            if directory_from_command_line:
                ...
                configuration['prevent_browse_view'] = True
                configuration['fullscreen'] = False
            # Only apply limit and filter for directory browsing
            if args.limit is not None:
                configuration['limit'] = args.limit
            if args.filter is not None:
                configuration['filter'] = args.filter
        ...
        if getattr(args, '_explicit_no_fullscreen_flag', False):
            # --no-fullscreen explicitly passed: do not restore fullscreen
            should_restore_fullscreen = False
        elif getattr(args, '_explicit_fullscreen_flag', False):
            should_restore_fullscreen = True
        elif hasattr(args, '_restore_state') and args._restore_state:
            restore_state = args._restore_state
            last_os_fullscreen = restore_state.get('last_os_fullscreen')
            if last_os_fullscreen is True:
                should_restore_fullscreen = True
        
        if should_restore_fullscreen:
            def try_fullscreen_direct():
                if not window.isFullScreen() and window.isVisible():
                    window.showFullScreen()
            QTimer.singleShot(300, try_fullscreen_direct)
        
        # Ensure menus are configured and focus is set on main pane immediately
        # This ensures menu keys work right away when the app opens
        def ensure_menus_and_focus():
            try:
                # Ensure menu actions are enabled and shortcuts registered
                if hasattr(window, 'menu_manager'):
                    window.menu_manager.initialize_menu_states()
                    # Re-register menu shortcuts with window (idempotent).
                    # On macOS, shortcuts may not work until window is shown.
                    window.menu_manager._register_menu_shortcuts_with_window()
                
                # Set focus on the main pane (thumbnail or browse mode)
        ...
        # Set focus immediately and also after a short delay to ensure it sticks
        ensure_menus_and_focus()
        QTimer.singleShot(50, ensure_menus_and_focus)
