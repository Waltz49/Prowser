...
# Third-party imports
from PySide6.QtCore import QSize, Qt, QTimer, QPoint, QPointF, QMimeData, QUrl
...
from utils import should_preserve_window_focus
...
class BrowseViewHandler:
    ...
    def __init__(self, window):
        ...
        self._browse_drag_start_pos = None
    ...
    def enter_macos_space_mode(self):
        ...
        if self.main_window.isFullScreen():
            QTimer.singleShot(
                200, self.main_window.menu_manager.update_macos_display_mode_menu
            )
            return
        ...
        if (
            ...
            and hasattr(self.main_window, 'image_container')
        ):
            QTimer.singleShot(
                150,
                lambda: self.main_window.image_container.resize(
                    self.get_effective_display_size()
                ),
            )
        QTimer.singleShot(
            200, self.main_window.menu_manager.update_macos_display_mode_menu
    ...
    def enter_windowed_mode(self):
        """Enter windowed display mode (exit macOS Space)."""
    ...
    def _browse_file_drag_path(self) -> str | None:
        """Current browse image path if it exists on disk."""
        mw = self.main_window
        if not hasattr(mw, 'get_current_image_path'):
            return None
    ...
    def _start_file_drag(self):
        """Drag the current browse image out to Finder or other apps."""
        mw = self.main_window
        image_path = self._browse_file_drag_path()
        if not image_path:
            return

        mime = QMimeData()
        mime.setUrls([QUrl.fromLocalFile(image_path)])
        mime.setText(image_path)
        uri_list = f"file://{image_path}"
        mime.setData('text/uri-list', uri_list.encode('utf-8'))
        mime.setData('text/plain', image_path.encode('utf-8'))

        drag_source = mw.image_label if getattr(mw, 'image_label', None) else mw
        drag = QDrag(drag_source)
        drag.setMimeData(mime)

        preview = None
        if getattr(mw, 'image_label', None):
            preview = mw.image_label.pixmap()
        if (not preview or preview.isNull()) and getattr(mw, 'current_pixmap', None):
            preview = mw.current_pixmap
        if preview and not preview.isNull():
            drag.setPixmap(preview.scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def handle_mouse_press(self, event):
        """Handle mouse press for panning and file drag in browse mode"""
        mw = self.main_window
        ...
        if btn == Qt.LeftButton:
            if self._browse_file_drag_path() or self.can_pan_image():
                self._browse_drag_start_pos = event.position().toPoint()
    ...
    def handle_mouse_move(self, event):
        """Handle mouse movement for panning and file drag in browse mode"""
        mw = self.main_window
        if mw.current_view_mode != 'browse':
            return False

        if self._browse_drag_start_pos is not None and not mw.is_dragging:
            current_pos = event.position().toPoint()
            distance = (current_pos - self._browse_drag_start_pos).manhattanLength()
            if distance > 5:
                modifiers = QApplication.keyboardModifiers()
                cmd_held = bool(modifiers & Qt.ControlModifier)
                image_path = self._browse_file_drag_path()
                want_file_drag = bool(
                    image_path and (not self.can_pan_image() or cmd_held)
                )
                if want_file_drag:
                    self._browse_drag_start_pos = None
                ...
                if self.can_pan_image():
                    mw.is_dragging = True
                    mw.drag_start_pos = self._browse_drag_start_pos
                    self._browse_drag_start_pos = None
    ...
    def handle_mouse_release(self, event):
        """Handle mouse release to stop panning or pending file drag in browse mode"""
        mw = self.main_window
        if mw.current_view_mode != 'browse':
            return False
        
        if event.button() == Qt.LeftButton:
            had_pending_drag = self._browse_drag_start_pos is not None
            self._browse_drag_start_pos = None
            if mw.is_dragging:
