...
from exif.exif_utils import get_exif_bytes_from_pil, format_supports_exif
from thumbnails.thumbnail_constants import ERROR_COLOR_HEX
from utils import (
    show_styled_warning,
...
def get_existing_conflict_paths(files: List[str], target_format: str) -> List[str]:
    """Source and existing target paths that block conversion (for conflict thumbnail view)."""
    return [path for _, path in get_existing_conflict_group_pairs(files, target_format)]


def get_existing_conflict_group_pairs(files: List[str], target_format: str) -> List[Tuple[str, str]]:
    """Group key + path pairs for duplicate-section presentation (source then target per group)."""
    target_ext = f'.{target_format.lower()}'
...
class ConvertFormatDialog(QDialog):
    ...
    def __init__(self, parent, files: List[str]):
        ...
        self._existing_conflict_paths: List[str] = []
        if parent is not None:
            parent.convert_format_pending_files = list(self.files)
    ...
    def _clear_convert_pending_files(self) -> None:
        parent = self.parent()
        if parent is not None:
            parent.convert_format_pending_files = None
    ...
    def setup_ui(self):
        ...
        # Buttons — Cancel on the left, primary actions on the right
        ...
        self.show_conflicts_button = QPushButton("Show Conflicts")
        self.show_conflicts_button.setFocusPolicy(Qt.StrongFocus)
        self.show_conflicts_button.setDefault(False)
        self.show_conflicts_button.setVisible(False)
        button_layout.addWidget(self.show_conflicts_button)
        ...
        self.show_conflicts_button.setStyleSheet(button_style)
        
        # Connect buttons
        self.convert_button.clicked.connect(self.accept)
        self.show_conflicts_button.clicked.connect(self.on_show_conflicts)
    ...
    def showEvent(self, event):
        """Set default button/focus based on whether Show Conflicts is offered."""
        super().showEvent(event)
        self._update_default_action_button()
    ...
    def check_conflicts(self):
        ...
        if conflict_messages:
            ...
            if existing_files:
                self._existing_conflict_paths = get_existing_conflict_paths(
                    self.files, self.saved_format
                )
                self.convert_button.setVisible(False)
                self.show_conflicts_button.setVisible(True)
                self.show_conflicts_button.setEnabled(bool(self._existing_conflict_paths))
            else:
                self._existing_conflict_paths = []
                self.convert_button.setVisible(True)
    ...
    def _update_default_action_button(self):
        """Show Conflicts is default when visible; otherwise Cancel."""
        use_show_conflicts = (
            self.show_conflicts_button.isVisible()
            and self.show_conflicts_button.isEnabled()
        )
        self.cancel_button.setDefault(not use_show_conflicts)
        self.convert_button.setDefault(False)
        self.show_conflicts_button.setDefault(use_show_conflicts)
        if use_show_conflicts:
            self.show_conflicts_button.setFocus()
        else:
    ...
    def on_show_conflicts(self):
        """Open duplicate-style grouped thumbnail view of conflicting source and target paths."""
        target_format = self.get_selected_format() or self.saved_format
        if not target_format:
            return

        group_path_pairs = get_existing_conflict_group_pairs(self.files, target_format)
        ...
        if main_window:
            from files.convert_conflict_auto_rename import (
                build_convert_conflict_context,
                resolve_single_conflict_pair_order,
            )
            ...
            conflict_count = len(group_path_pairs)
            convert_context = build_convert_conflict_context(
                self.files, target_format, group_path_pairs
            )
            _present_duplicate_groups_browse_view(
                main_window,
                group_path_pairs,
                current_image_path,
                f"Convert conflicts ({conflict_count} files)",
                auto_select=False,
    ...
    def done(self, result: int) -> None:
        self._clear_convert_pending_files()
        super().done(result)
