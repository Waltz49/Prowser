...
from files.tree_image_discovery import (
    ...
    build_tree_image_check_context,
)
...
class CustomTreeView(QTreeView):
    ...
    def _handle_delayed_single_click(self) -> None:
        ...
        if self.main_window and hasattr(self.main_window, "file_tree_handler"):
            ...
            if handler.is_tree_initialized():
                index = handler._get_proxy_index(path)
    ...
    def _handle_all_images(self, directory_path: str) -> None:
        ...
        # Scan directory recursively for all images (Shift-A shows every image, not browse filter)
        ...
        if len(image_files) > 100:  # prepopulate cache if there are more than 100 images
            ...
            # Prepopulate cache with progress dialog (user can cancel)
        ...
        # Open as specific files view (skip browse filter_pattern — user asked for all images)
        configuration = {
            'files': image_files,
            'skip_filter_pattern': True,
            'force_specific_files_grid': len(image_files) == 1,
        }
    ...
    def _handle_clear_cache(self, directory_path: str) -> None:
        ...
        try:
            ...
            try:
                from faces.face_cache import clear_face_cache_for_directory
    ...
    def _handle_copy_file_path(self, directory_path: str) -> None:
        ...
        try:
            ...
            copy_text_to_clipboard(
                normalize_path_for_display(directory_path),
                anchor=tree,
    ...
    def _execute_clip_search_execution(self, text_prompt: str, recursive: bool, threshold: float, search_directory: str) -> None:
            ...
            for root, dirs, files in os.walk(search_dir):
                ...
                for file in files:
                    ...
                    if get_file_extension(file) in image_extensions and os.path.isfile(file_path):
                        ...
                        if not path_matches_active_filter(self.main_window, abs_path):
                            continue
        ...
        try:
            ...
            if hasattr(self.main_window, 'sorting_manager') and getattr(self.main_window, 'filter_pattern', None):
                new_displayed_images = self.main_window.sorting_manager.filter_images_by_pattern(new_displayed_images)
...
class CustomFileSystemFilter(QSortFilterProxyModel):
    ...
    def __init__(self, parent: Optional[QObject] = None, main_window: Optional[Any] = None) -> None:
        ...
        self._enabled_root_dirs: Set[str] = set()
        self._root_directories_with_slash: Set[str] = set()
        ...
        self._invalidate_debounce_timer: Optional[QTimer] = None
    ...
    def attach_discovery_service(self, service: Optional[TreeImageDiscoveryService]) -> None:
        ...
        if self._discovery_service is service:
            return
        if self._discovery_service:
            try:
                self._discovery_service.result_ready.disconnect(self._on_deep_check_result)
            except Exception:
                pass
        self._discovery_service = service
        if service:
            service.result_ready.connect(
                self._on_deep_check_result,
                Qt.ConnectionType.QueuedConnection,
            )

    def refresh_filter_settings(self) -> None:
        """Refresh cached settings used by filterAcceptsRow (call when settings change)."""
        self._context_generation += 1
        self._enabled_root_dirs = get_enabled_root_directories()
        self._root_directories_with_slash = self._enabled_root_dirs | {"/"}
    ...
    def _schedule_invalidate_filter(self) -> None:
        ...
        if self.main_window:
            handler = getattr(self.main_window, "file_tree_handler", None)
        if handler and (
            getattr(handler, "_expand_in_progress", False)
            or getattr(handler, "_rebuild_in_progress", False)
        ):
            handler._deferred_filter_invalidate = True
            return
        if not self._invalidate_debounce_timer:
            self._invalidate_debounce_timer = QTimer()
            self._invalidate_debounce_timer.setSingleShot(True)
            self._invalidate_debounce_timer.timeout.connect(self._invalidate_filter_now)
        if not self._invalidate_debounce_timer.isActive():
            self._invalidate_debounce_timer.start(50)

    def _clear_image_caches(self) -> None:
        self.has_images_cache.clear()
        self._pending_deep_checks.clear()
    ...
    def _get_cached_has_images(self, dir_path: str) -> Optional[bool]:
        cache_key = self._make_cache_key(dir_path)
        if cache_key in self.has_images_cache:
            return self.has_images_cache[cache_key]
        return None

    def _set_cached_has_images(self, dir_path: str, found: bool) -> None:
        cache_key = self._make_cache_key(dir_path)
    ...
    def _expanded_dirs_for_priority(self) -> Set[str]:
        ...
        if self._in_filter_accept_row:
            return set()
        if self._expanded_dirs_cache is not None:
            return self._expanded_dirs_cache
        expanded: Set[str] = set()
        handler = None
        if self.main_window:
            handler = getattr(self.main_window, "file_tree_handler", None)
        if handler and handler.is_tree_initialized():
            expanded = set(handler.get_expanded_directories())
        self._expanded_dirs_cache = expanded
        return expanded

    def _deep_check_priority_for_path(self, dir_path: str) -> int:
        """Lower value = higher priority. 0: navigation path, 1: expanded/nearby, 2: default."""
        norm = os.path.normpath(dir_path)
        for p in self.priority_paths:
            pn = os.path.normpath(p)
            if norm == pn:
                return 0
            if norm.startswith(pn + os.sep) or pn.startswith(norm + os.sep):
                return 1

        for en in self._expanded_dirs_for_priority():
            if norm == en or norm.startswith(en + os.sep) or en.startswith(norm + os.sep):
                return 1
            if os.path.dirname(norm) == os.path.dirname(en):
                return 1
        return 2

    def _schedule_deep_image_check(self, dir_path: str, *, priority: Optional[int] = None) -> None:
        if not self._discovery_service or not self._check_context:
            return
        cache_key = self._make_cache_key(dir_path)
        if cache_key in self.has_images_cache:
            return
        if priority is None:
            priority = self._deep_check_priority_for_path(dir_path)
        context = self._check_context

        if cache_key in self._pending_deep_checks:
            self._discovery_service.submit(dir_path, cache_key, context, priority)
            return

        self._pending_deep_checks.add(cache_key)
    ...
    def _on_deep_check_result(self, dir_path: str, cache_key: str, found: bool) -> None:
        self._pending_deep_checks.discard(cache_key)
        if cache_key in self.has_images_cache:
            return
        if self._check_context and cache_key != self._check_context.cache_key_for(dir_path):
            return
        self.has_images_cache[cache_key] = found
        # Re-filter to show/hide rows (icon color uses shallow check only, not deep cache).
        self._schedule_invalidate_filter()

    def _folder_icon_has_matching_images(self, file_path: str, mode: str) -> bool:
        """Yellow only when this directory directly contains matching images (not subdirs)."""
        if mode == "all":
            return True
        try:
            return self._quick_check_matching_images(file_path)
        except Exception:
            return False

    def _directory_has_images_for_filter(
        self,
        file_path: str,
        mode: str,
        *,
        allow_pending_visible: bool = False,
    ) -> bool:
        """Filter visibility; deep checks run in background."""
        if mode == "all":
            return True
        if not self._should_check_for_images(file_path):
            return True

        cached = self._get_cached_has_images(file_path)
        if cached is not None:
            return cached

        if self._quick_check_matching_images(file_path):
            self._set_cached_has_images(file_path, True)
            return True

        cache_key = self._make_cache_key(file_path)
        if cache_key not in self._pending_deep_checks:
            prio = 0 if allow_pending_visible else None
            self._schedule_deep_image_check(file_path, priority=prio)
        # Show while deep check runs; hide after if no matching images (use_filter/images).
        return True
    ...
    def _paint_folder_icon(
        ...
        if rename_status is True:
            ...
            green_pen = QPen(QColor(0, 255, 0), 3)
        ...
        try:
            for entry in os.listdir(dir_path):
                ...
                if not os.path.isfile(entry_path):
                    continue
                if get_file_extension(entry) not in get_image_extensions():
                    continue
                if mode == "images":
                    return True
                if mode == "use_filter" and self._filename_matches_filter_pattern(entry, mode):
                    return True
        except (OSError, PermissionError):
            return False
        return False
    ...
    def _should_check_for_images(self, dir_path: str) -> bool:
        return dir_path not in self._root_directories_with_slash

    def filterAcceptsRow(self, source_row: int, source_parent: QModelIndex) -> bool:
        self._in_filter_accept_row = True
        try:
            return self._filter_accepts_row_impl(source_row, source_parent)
        finally:
            self._in_filter_accept_row = False

    def _filter_accepts_row_impl(self, source_row: int, source_parent: QModelIndex) -> bool:
        ...
        if not self._get_follow_symlinks_cached():
            ...
            if is_symlink and not is_enabled_root_symlink(normalized_path, self._enabled_root_dirs):
                return False
        ...
        if in_priority_paths:
            if mode == 'use_filter':
                if self._should_check_for_images(file_path) and not self._directory_has_images_for_filter(
                    file_path, mode, allow_pending_visible=True,
                ):
                    return False
            return True
        ...
        if any(filename_lower.endswith(ext) for ext in EXCLUDED_EXTENSIONS):
            return False
        if self._cached_always_show_work and filename_lower == 'work':
            return True
        if mode != 'all':
            if self._should_check_for_images(file_path) and not self._directory_has_images_for_filter(
                file_path, mode, allow_pending_visible=False,
            ):
                return False
        return True
            ...
            if source_index.isValid() and source_model:
                ...
                if file_info.isDir():
                    ...
                    if mode == 'all':
                        ...
                        has_matching_images = self._folder_icon_has_matching_images(file_path, mode)
                    ...
                    cache_key = (
                        ...
                        has_matching_images,
                        has_border,
                        rename_status is True,
                    )
                    cached_icon = self._icon_cache.get(cache_key)
                    if cached_icon is not None:
                        return cached_icon

                    icon = self._paint_folder_icon(
                        folder_color,
                        has_border,
                        border_color,
                        rename_status if rename_status is True else None,
                    )
                    if len(self._icon_cache) >= self._max_icon_cache_size:
                        oldest = next(iter(self._icon_cache))
                        del self._icon_cache[oldest]
...
class FileTreeHandler(QObject):
    ...
    def __init__(self, main_window: Any, parent_widget: QWidget) -> None:
        ...
        self._deferred_filter_invalidate: bool = False
        self._getting_expanded_directories: bool = False
    ...
    def _refresh_config_cache(self) -> None:
        ...
        if self.filter_proxy:
            self.filter_proxy.refresh_filter_settings()

    def _ensure_image_discovery_service(self) -> None:
        if self._image_discovery_service is None:
            # No QObject parent — shutdown explicitly before app exit (avoids QThread destroy crash).
            self._image_discovery_service = TreeImageDiscoveryService()
        if self.filter_proxy:
            self.filter_proxy.attach_discovery_service(self._image_discovery_service)

    def shutdown_image_discovery(self) -> None:
        """Stop background image discovery thread (call before exit)."""
        service = getattr(self, "_image_discovery_service", None)
        if not service:
            return
        if self.filter_proxy:
            self.filter_proxy.attach_discovery_service(None)
        try:
            service.shutdown()
        except Exception:
            pass
        try:
            ...
            pass
        self._image_discovery_service = None

    def _flush_deferred_filter_invalidate(self) -> None:
        if not getattr(self, "_deferred_filter_invalidate", False):
            return
        self._deferred_filter_invalidate = False
        if self.filter_proxy:
            self.filter_proxy._schedule_invalidate_filter()
    ...
    def rebuild_tree(self, show_hidden: Optional[bool] = None) -> None:
        ...
        try:
            ...
            if self.filter_proxy:
                self.filter_proxy._clear_image_caches()
                self.filter_proxy.refresh_filter_settings()
            ...
            self._flush_deferred_filter_invalidate()
    ...
    def get_expanded_directories(self) -> List[str]:
        ...
        if getattr(self, "_getting_expanded_directories", False):
            return list(getattr(self, "_expanded_directory_paths", set()))
        expanded = []
        if not self.file_tree or not self.file_model or not self.filter_proxy:
            return expanded
        self._getting_expanded_directories = True
        try:
            root_index = self.file_tree.rootIndex()
            ...
            self._getting_expanded_directories = False
        self._expanded_directory_paths = set(expanded)
        if self.filter_proxy:
            self.filter_proxy._expanded_dirs_cache = set(expanded)
        return expanded
    ...
    def _on_tree_item_expanded(self, index: QModelIndex) -> None:
        ...
        try:
            if self.filter_proxy and self.file_model:
                source_index = self.filter_proxy.mapToSource(index)
                if source_index.isValid():
                    dir_path = self.file_model.filePath(source_index)
                    if dir_path and os.path.isdir(dir_path):
                        self._expanded_directory_paths.add(os.path.normpath(dir_path))
                        self.filter_proxy._expanded_dirs_cache = set(self._expanded_directory_paths)
                ...
                if source_index.isValid():
                    ...
                    if dir_path:
                        ...
                        self._expanded_directory_paths = {
                            p for p in self._expanded_directory_paths
                            if p != norm and not p.startswith(norm + os.sep)
                        }
                        self.filter_proxy._expanded_dirs_cache = set(self._expanded_directory_paths)
    ...
    def open_settings_to_max_images(self) -> None:
        """Open settings dialog to General tab"""
        if not self.main_window:
            return
        # Show settings dialog with General tab (index 0)
    ...
    def expand_file_tree(self, depth=None):
        ...
        import traceback
        import datetime
        from thumbnails.thumbnail_constants import EXPANSION_LEVELS
        from config import get_config
        ...
        try:
            ...
            def expand_discovered_directories():
                ...
                def expand_single_path(dir_path):
                    ...
                    try:
                        ...
                        if fresh_source_idx.isValid():
                            ...
                            if fresh_proxy_idx and fresh_proxy_idx.isValid():
                                try:
                                    if file_tree.isExpanded(fresh_proxy_idx):
                ...
                def expand_next_batch(start_idx=0):
                    ...
                    # Process events after batch — exclude user input to avoid stale-index expand/collapse
                    QApplication.processEvents(QEventLoop.ProcessEventsFlag.ExcludeUserInputEvents)
