...
def _align_flux_dims(w: int, h: int, *, max_side: int = FLUX_LOCAL_IMAGE_DIM_MAX) -> Tuple[int, int]:
    w = max(256, min(max_side, (int(w) // 8) * 8))
    h = max(256, min(max_side, (int(h) // 8) * 8))
    return w, h


def align_mflux_dims(
    w: int, h: int, *, max_side: int = FLUX_LOCAL_IMAGE_DIM_MAX
) -> Tuple[int, int]:
    w, h = _align_flux_dims(w, h, max_side=max_side)
