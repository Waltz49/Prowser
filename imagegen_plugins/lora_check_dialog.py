...
def _progress_html(
    ...
    lora_label = lora_choice_label(entry) if entry else lora_id
    if phase == "download":
...
def run_check_loras_dialog(parent) -> None:
    ...
    probes_total = count_local_lora_probes(entries)
    if probes_total == 0 and not any(probe_models_for_lora_entry(e) for e in entries):
        show_styled_information(
            parent,
            "Check LoRAs",
