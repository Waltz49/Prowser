...
from image_extensions_helpers import get_image_extensions, MIN_THUMBNAIL_SIZE
from feature_cache_manager import FeatureCacheManager
from cnn_image_similarity_sorter import CNNImageSimilaritySorter
from background_thumbnail_cache import BackgroundThumbnailCache
from worker_image_loader import load_thumbnail_pil_with_exif_correction


def _flush_face_cache_index_safe() -> None:
    """Persist batched face cache index writes (see face_cache.INDEX_PERSIST_INTERVAL)."""
    try:
        from face_cache import flush_face_cache_index
        ...
        try:
            ...
            for idx, image_path in enumerate(image_files):
                ...
                try:
                    ...
                    if extract_faces:
                        try:
                            from face_gathering_coordinator import is_foreground_face_scan_active
                            if not is_foreground_face_scan_active():
                                ...
                                from face_engine import encode_faces_from_path as face_encode
                                from face_cache import has_cached_faces as face_has_cached
                                from face_cache import set_encodings as face_set_encodings
