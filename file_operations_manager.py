...
from thumbnail_constants import GREEN, RED, RESET, get_image_extensions
from undo_applescript_fix import AppleScriptUndoManager
from utils import (
    styled_message_box,
...
def _present_duplicate_groups_browse_view(
    ...
    configuration = {"files": duplicate_displayed}
...
class FileOperationsManager:
    ...
    def _migrate_face_metadata_after_rename(
        ...
        from face_sample_cache import migrate_path_in_index
        from known_faces_manager import update_sample_paths_for_rename
    ...
    def rename_with_custom_prefix(self, bypass_dialog=False, prefix_template=None, 
        ...
        try:
            ...
            # Check if there's a limit and displayed_images is at that limit
            # Show confirmation dialog AFTER user has configured and saved rename settings (skip if bypassing dialog)
            if not bypass_dialog and hasattr(mw, 'limit') and mw.limit and mw.limit > 0:
                if len(displayed_images) >= mw.limit:
                    # Check total files in directory to see if limit is actually limiting
                    if hasattr(mw, 'count_total_files_in_directory') and hasattr(mw, 'current_directory') and mw.current_directory:
                        total_files = mw.count_total_files_in_directory(mw.current_directory)
                        if total_files > mw.limit:
                            # Limit is active and there are more files than displayed
                            result = show_styled_question(
                                mw,
                                "Rename Selected Files Only",
                                f"Only {len(selected_files)} selected {file_string(len(selected_files))} will be renamed.\n\n"
                                f"The directory contains {total_files} files, but only {len(displayed_images)} are displayed due to the limit.\n\n"
                                f"Continue with renaming only the selected files?",
                                default_no=False
                            )
                            if result != QMessageBox.StandardButton.Yes:
                                # User cancelled - settings are already saved, just exit
                                restart_background_loading()
                                return
    ...
    def quick_mass_rename(self):
        ...
        Only available when: thumbnail mode, non-specific files mode, and limit=unlimited (99999).
        ...
        if not (hasattr(mw, 'limit') and mw.limit == 99999):
            current_limit = getattr(mw, 'limit', 'unknown')
            show_styled_warning(
                mw,
                "Quick Mass Rename",
                f"Quick Mass Rename is not available when a limit is set.\n\n"
                f"Current limit: {current_limit}\n"
                f"Please set the limit to unlimited (0) to use this feature.",
            )
            return
