...
# Module-level cache for noimage placeholder compositing
_noimage_source_cache = None
...
def load_noimage_thumbnail(size: int) -> QPixmap:
    """
    Load the noimage placeholder and scale it to the requested thumbnail size.
    Applies transparency background (checkerboard or solid color) based on settings.
    Uses caching to load the source image only once per settings combination.
    
    Args:
        size: Thumbnail size (width and height)
        
    Returns:
        QPixmap of the placeholder scaled to the requested size with appropriate background,
        or a blank pixmap if loading fails
    """
    global _noimage_source_cache, _noimage_composited_cache, _noimage_cache_settings
    
    from theme.theme_base import asset_path

    noimage_path = asset_path("noimage.png")
    if not os.path.exists(noimage_path):
        # Fallback: return a blank pixmap if noimage.png doesn't exist
    ...
    # Load and cache the source pixmap (only once)
    if _noimage_source_cache is None:
        pixmap = QPixmap(noimage_path)
        if pixmap.isNull():
            return QPixmap(size, size)
        _noimage_source_cache = pixmap
    
    # Check if the cached source has transparency
    has_alpha = _noimage_source_cache.toImage().hasAlphaChannel()
    
    # If no transparency, just scale and return
    if not has_alpha:
        scaled_pixmap = _noimage_source_cache.scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        if scaled_pixmap.isNull():
            return QPixmap(size, size)
        return scaled_pixmap
    
    # For transparent placeholders, composite with background
    # Use a reasonable base size for compositing (we'll scale the result)
    # This avoids recompositing for every size request
    base_size = 512  # Matches noimage.png resolution
    
    # Check if we need to recomposite (cache miss or settings changed)
    if _noimage_composited_cache is None:
        # Scale source to base size for compositing
        scaled_source = _noimage_source_cache.scaled(
        ...
        # Composite the scaled placeholder on top of the background
        painter = QPainter(background_pixmap)
        painter.setRenderHint(QPainter.Antialiasing, True)
        painter.drawPixmap(0, 0, scaled_source)
        painter.end()
...
def get_image_dimensions_and_exif_date(image_path: str) -> Optional[Tuple[Tuple[int, int], Optional[float]]]:
    ...
    """
    try:
        from files.cr2_raw_loader import is_cr2_path, get_cr2_dimensions_from_raw
        if is_cr2_path(image_path):
            from exif.exif_utils import get_exif_orientation_from_path, get_exif_timestamp_from_image_path
        ...
        from PIL import Image
        from exif.exif_utils import get_exif_orientation_from_pil, get_exif_timestamp_from_pil
