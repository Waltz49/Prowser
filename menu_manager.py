...
# Third-party imports
from PySide6.QtCore import Qt, QTimer, QSize, QMutexLocker
...
from PySide6.QtWidgets import QApplication, QMenu, QWidget, QHBoxLayout, QWidgetAction
...
from thumbnail_constants import RED, RESET, get_image_extensions
from utils import (
...
class MenuManager:
    ...
    def setup_actions(self):
        ...
        # Create menu (optional imagegen plugins)
        try:
        ...
        # Register all menu actions with main window so shortcuts work immediately.
    ...
    def _setup_view_menu(self, menubar):
        ...
        from theme_service import apply_theme, connect_system_theme_listener, set_theme_main_window, sync_view_theme_menu_actions
        ...
        native_fullscreen_separator_action = QWidgetAction(self.main_window)
        native_fullscreen_separator_action.setDefaultWidget(TextSeparator("Fullscreen"))
        view_menu.addAction(native_fullscreen_separator_action)

        # Native Fullscreen (macOS only)
        if hasattr(self.main_window, 'MACOS_FULLSCREEN_AVAILABLE') and self.main_window.MACOS_FULLSCREEN_AVAILABLE:
            self.main_window.native_fullscreen_action = QAction("Fullscreen (macOS)", self.main_window)
            self.main_window.native_fullscreen_action.setShortcuts([
                # QKeySequence("F11"),
                QKeySequence("Ctrl+Meta+F")
            ])
            self.main_window.native_fullscreen_action.setCheckable(True)
            self.main_window.native_fullscreen_action.triggered.connect(self.main_window.toggle_fullscreen)
            view_menu.addAction(self.main_window.native_fullscreen_action)
    ...
    def _setup_move_menu(self, menubar):
        """Setup Move menu"""
        self.move_menu = menubar.addMenu("Move to")
    ...
    def _debug_random_images(self):
        ...
        from random_images_from_recents import run_random_images_from_recents
    ...
    def update_native_fullscreen_checkbox(self):
        """Update the macOS fullscreen menu checkbox state"""
        if hasattr(self.main_window, 'native_fullscreen_action') and self.main_window.native_fullscreen_action:
            try:
                is_fullscreen = is_macos_spaces_fullscreen()
                self.main_window.native_fullscreen_action.setChecked(is_fullscreen)
            except RuntimeError:
    ...
    def update_edit_menu_states(self):
        ...
        if (hasattr(mw, 'file_tree_handler') and 
            ...
            from file_tree_handler import CustomTreeView
    ...
    def update_tools_menu_states(self):
        ...
        if hasattr(mw, 'examine_image_action'):
            try:
                from face_engine import is_available as face_available
        ...
        if hasattr(mw, 'backup_custom_sort_action') and hasattr(mw, 'restore_custom_sort_action'):
            # Only show in thumbnail mode, not specific files mode, and limit is unlimited (99999)
            is_thumbnail_mode = (hasattr(mw, 'current_view_mode') and 
                               mw.current_view_mode == 'thumbnail')
            is_specific_files_mode = getattr(mw, 'specific_files_active', False)
            is_unlimited = (hasattr(mw, 'limit') and mw.limit == 99999)
    ...
    def update_view_menu_enabled_states(self):
        ...
        self.update_native_fullscreen_checkbox()
        ...
        if config is None:
            ...
            from thumbnail_constants import GREEN, RESET, RED
