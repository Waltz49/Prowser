...
def _apply_active_filter_to_results(mw, image_paths: List[str]) -> List[str]:
    """Keep only paths that match the active filter pattern."""
    if hasattr(mw, 'sorting_manager') and getattr(mw, 'filter_pattern', None):
        return mw.sorting_manager.filter_images_by_pattern(image_paths)
    return image_paths
...
def reorder_images_by_similarity(mw):
    ...
    if recursive:
        ...
        for source_img in source_images_set:
            if source_img not in displayed_images_set and path_matches_active_filter(mw, source_img):
        ...
        for root, dirs, files in os.walk(search_dir):
            ...
            for file in files:
                ...
                if ext in image_extensions and os.path.isfile(file_path):
                    ...
                    if not path_matches_active_filter(mw, abs_path):
                        continue
    ...
    # Add source images if not already present and they match the filter
    for source_img in source_images_set:
        if source_img not in displayed_images_set and path_matches_active_filter(mw, source_img):
            displayed_images.append(source_img)
            displayed_images_set.add(source_img)
    ...
    try:
        ...
        source_images_list = [
            img for img in source_images_set
            if path_matches_active_filter(mw, img)
        ]
        ...
        new_displayed_images = _apply_active_filter_to_results(mw, new_displayed_images)
