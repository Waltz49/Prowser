...
_REF_IMAGE_EXT_LOWER = frozenset(ext.lower() for ext in _REF_IMAGE_EXTENSIONS)
...
def _alternate_extension_paths(path: str) -> List[str]:
    """Same basename with other known image extensions (when *path* has one)."""
    root, ext = os.path.splitext(path)
    if not ext or ext.lower() not in _REF_IMAGE_EXT_LOWER:
        return []
    alts: List[str] = []
    seen_lower: Set[str] = {ext.lower()}
    for alt_ext in _REF_IMAGE_EXTENSIONS:
        alt_lower = alt_ext.lower()
        if alt_lower in seen_lower:
            continue
        seen_lower.add(alt_lower)
        alts.append(root + alt_ext)
    return alts


def _reference_path_passes_mtime(
    cand: str, expected_mtime: Optional[float]
) -> bool:
    if expected_mtime is None:
        return True
    try:
        return (
            abs(os.path.getmtime(cand) - expected_mtime) <= _REF_FILEDATE_TOLERANCE_S
        )
    except OSError:
        return False


def resolve_reference_path_with_swap(
    ...
    for cand in candidates:
        if os.path.isfile(cand):
            if _reference_path_passes_mtime(cand, expected_mtime):
                return cand, False
            continue
        for alt in _alternate_extension_paths(cand):
            if os.path.isfile(alt) and _reference_path_passes_mtime(alt, expected_mtime):
                return alt, True
    return None, False
...
def resolve_reference_path(
    ...
    path, _swapped = resolve_reference_path_with_swap(
        image_dir, fname, expected_mtime
    )
    return path


def notify_reference_extension_swap(main_window) -> None:
    """Status message when EXIF references resolved via a different extension."""
    if main_window is None:
        return
    notifier = getattr(main_window, "status_notification", None)
    if notifier is None:
        return
    notifier.show_message(
        "Some references were found with a different file extension",
        duration=5000,
    )
...
def resolve_reference_entries_map(
    ...
    if not current_path or not os.path.isfile(current_path):
        return resolved, extension_swapped
    for fname, expected_mtime in entries:
        key = fname.strip().lower()
        if key in resolved:
            continue
        path, swapped = resolve_reference_path_with_swap(
            image_dir, fname, expected_mtime
        )
        if path:
            resolved[key] = path
            extension_swapped = extension_swapped or swapped
    return resolved, extension_swapped
...
def collect_reference_chain_paths(
    ...
    def visit(path: str, direct_entries: List[Tuple[str, Optional[float]]]) -> None:
        ...
        resolved, swapped = resolve_reference_entries_map(
...
def path_is_referenced_in_exif(candidate_path: str, target_path: str) -> bool:
    ...
    if not entries:
        return False
    resolved, _swapped = resolve_reference_entries_map(image_dir, candidate_path, entries)
...
def has_resolvable_exif_references(image_path: str) -> bool:
    ...
    resolved, _swapped = resolve_reference_entries_map(image_dir, image_path, entries)
...
def open_reference_graph_for_path(main_window, image_path: str) -> None:
    ...
    image_dir = os.path.dirname(image_path) or ""
    entries = get_reference_entries_for_path(image_path)
    paths, extension_swapped = collect_reference_chain_paths(
...
def valid_exif_reference_paths_for_image(
    ...
    if not entries:
        return [image_path]
    image_dir = os.path.dirname(image_path) or ""
    paths, _resolved, _swapped = resolve_exif_reference_paths(
        image_dir, image_path, entries
    )
    out: List[str] = []
    seen: Set[str] = set()
...
def resolve_exif_reference_paths(
    ...
    resolved, extension_swapped = resolve_reference_entries_map(
    ...
    for fn, _expected_mtime in entries:
        ...
        if pn not in seen_paths:
            out.append(p)
            seen_paths.add(pn)
    return out, resolved, extension_swapped
...
def build_reference_graph(
    ...
    for target_path in canonical:
        ...
        resolved, _swapped = resolve_reference_entries_map(image_dir, target_path, entries)
