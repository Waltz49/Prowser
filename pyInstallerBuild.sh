...
create_spec_file() {
    ...
    # imagegen_plugins paths (absolute SCRIPT_DIR for PyInstaller analysis)
    ...
    # Extract directives using Python and handle empty lists properly
    DIRECTIVES=$(python -c "
import json
import os
script_dir = os.environ.get('SCRIPT_DIR', os.getcwd())
...
# Add mandatory PyObjC framework imports for macOS app selection functionality
# These are needed for settings_dialog.py editor selection feature
# Note: Only import parent modules - PyInstaller will automatically include all symbols
# (NSOpenPanel, NSWorkspace, etc. are classes/functions, not Python submodules)
mandatory_pyobjc_imports = [
    'AppKit',
    'LaunchServices',
    'CoreServices',
    'Foundation'
...
# face_recognition + face_recognition_models: optionally imported in face_engine; ensure always bundled
mandatory_hidden_imports = ['face_recognition', 'face_recognition_models', 'pyinstaller_frozen_support']

# imagegen_plugins: local package (Create menu); paths from pyinstaller_imagegen_paths.py
import json as _json
import os as _os
_imagegen_json = _os.environ.get('IMAGEGEN_HIDDEN_JSON', '[]')
try:
    imagegen_hidden = _json.loads(_imagegen_json)
except _json.JSONDecodeError:
    imagegen_hidden = ['imagegen_plugins']
if not imagegen_hidden:
    imagegen_hidden = ['imagegen_plugins']

mflux_hidden = ['mflux']
mlx_hidden = ['mlx', 'mlx._reprlib_fix']
diffusers_hidden = ['diffusers', 'diffusers.pipelines', 'diffusers.pipelines.sana']
accelerate_hidden = ['accelerate']
transformers_hidden = ['transformers']
requests_hidden = ['requests']

# Merge hidden imports, ensuring PyObjC and face_recognition modules are always included
all_hidden_imports = list(set(hidden_imports + mandatory_pyobjc_imports + mandatory_hidden_imports + imagegen_hidden + mflux_hidden + mlx_hidden + diffusers_hidden + accelerate_hidden + transformers_hidden + requests_hidden))

# face_recognition_models + mflux/MLX + diffusers stack for Create-menu generation
mandatory_collect_all = [
    'face_recognition_models', 'mflux', 'mlx', 'diffusers', 'accelerate',
    'transformers', 'requests', 'huggingface_hub', 'safetensors', 'regex', 'tokenizers',
]
collect_all = list(set(collect_all + mandatory_collect_all))

# Add Windows-specific exclusions for macOS-only builds
windows_excludes = [
    'win32com', 'win32api', 'win32con', 'win32gui', 'win32print',
    'win32process', 'win32security', 'win32service', 'win32serviceutil',
    'win32timezone', 'win32traceutil', 'win32ui', 'win32wnet',
    'pywintypes', 'pythoncom', 'winreg', 'msilib', 'msvcrt'
]

# Merge excludes, removing duplicates
all_excludes = list(set(excludes + windows_excludes))

print(f\"HIDDEN_IMPORTS={' '.join(all_hidden_imports) if all_hidden_imports else ''}\")
print(f\"COLLECT_ALL={' '.join(collect_all) if collect_all else ''}\")
print(f\"EXCLUDES={' '.join(all_excludes) if all_excludes else ''}\")
")
    ...
    PYINSTALLER_CMD="pyinstaller --name \"$APP_NAME\" --onedir --windowed --clean --noconfirm --log-level WARN --icon \"$ICON_FILE\" --add-data \"$ICON_FILE:.\" --argv-emulation --paths \"$SCRIPT_DIR\" --additional-hooks-dir \"$SCRIPT_DIR/pyinstaller_hooks\" --collect-submodules imagegen_plugins --collect-submodules mflux --collect-submodules mlx --collect-submodules diffusers --collect-submodules accelerate"
...
customize_spec_file() {
    ...
    # Read the generated directives for the custom spec
    DIRECTIVES=$(python -c "
import json
with open('pyinstaller_directives.json', 'r') as f:
    data = json.load(f)
...
# imagegen_plugins: local package (Create menu)
import json as _json
import os as _os
_imagegen_json = _os.environ.get('IMAGEGEN_HIDDEN_JSON', '[]')
try:
...
print(f\"HIDDEN_IMPORTS={repr(all_hidden_imports)}\")
print(f\"COLLECT_ALL={repr(collect_all)}\")
print(f\"EXCLUDES={repr(all_excludes)}\")
")
...
# COLLECT_ALL from directives was never merged into Analysis; collect imagegen backends here.
_imagegen_collect_datas = []
_imagegen_collect_binaries = []
_imagegen_collect_hidden = []
for _pkg in (
    'diffusers', 'accelerate', 'mflux', 'mlx', 'face_recognition_models',
    'transformers', 'requests', 'huggingface_hub', 'safetensors', 'regex', 'tokenizers',
):
    try:
...
for _pkg in (
    'transformers', 'requests', 'diffusers', 'huggingface_hub', 'accelerate', 'tokenizers',
):
    try:
...
verify_build() {
    ...
    if [ -d "$BUILD_DIR/${APP_NAME}.app" ]; then
        ...
        if find "$BUILD_DIR/${APP_NAME}.app" -path "*/diffusers/pipelines/*" -name "*.py" 2>/dev/null | head -1 | grep -q .; then
        ...
        if find "$BUILD_DIR/${APP_NAME}.app" -path "*/requests/__init__.py" 2>/dev/null | head -1 | grep -q .; then
...
cleanup() {
    ...
    # Note: .pyinstaller_face_models/ is preserved so the face model is reused on future builds
