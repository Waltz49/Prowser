...
def build_downloading_models_html(expanded: Set[str]) -> str:
    ...
    parts: list[str] = [
        ...
        p(
            ...
            f"{html.escape(COPY_ICON)}</span> to copy a script to the clipboard."
        ),
    ]
    ...
    *,
    ...
    if show_copy:
        copy_cell = (
            ...
            f'title="Copy to clipboard">{html.escape(COPY_ICON)}</a>'
            f"</td>"
        )
    return (
    ...
    def _show_copy_feedback(self) -> None:
        ...
        label.setStyleSheet(
            ...
            QLabel {{
                ...
                border-radius: 4px;
                padding: 4px 10px;
                font-size: {_HELP_COPY_FEEDBACK_PT};
            }}
            """
        )
        label.adjustSize()
        label.move(browser.width() - label.width() - 12, 10)
        label.show()
        label.raise_()
        ...
        QTimer.singleShot(1200, hide_label)
