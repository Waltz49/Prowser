...
from utils import show_styled_question, show_styled_warning, get_file_extension
...
class DirectoryLoader:
    ...
    def _do_load_directory(self, directory: str, external_load: bool = False, refresh_mode: bool = False):
        """Load images from directory with ULTRA-FAST startup and proper limit handling"""
        ...
        # Check for locked files - if they exist, ensure limit is unlimited (99999)
        ...
        if hasattr(self.main_window, 'lock_manager') and self.main_window.lock_manager:
            ...
            if has_locked_files and hasattr(self.main_window, 'limit') and self.main_window.limit < 99999:
                ...
                reply = show_styled_question(
                    ...
                    f"This directory contains locked files, which requires unlimited display (limit=0).\n\n"
                    f"Current limit: {self.main_window.limit}\n\n"
                    f"Would you like to set limit to unlimited and refresh?",
                    default_no=False,
                )
                if reply == QMessageBox.StandardButton.Yes:
                    self.main_window.limit = 99999
                    # Continue loading with unlimited limit
                else:
                    # User declined - still load but locked files may not be visible
                    pass
        ...
        # Apply limit if we have more images than the limit
        if len(current_files_list) > self.main_window.limit:
            if self.main_window.is_reversed:
                current_files_list = current_files_list[-self.main_window.limit:]
            else:
                current_files_list = current_files_list[:self.main_window.limit]
    ...
    def load_file_with_directory_thumbnails(self, target_file: str, limit: Optional[int] = None, external_load: bool = False):
        ...
        if limit == 99999:
            effective_limit = self.main_window.limit
        else:
            self.main_window.limit = limit
        ...
        if len(all_images) > effective_limit:
            ...
            self.main_window.displayed_images = all_images[start_index:end_index]
        else:
    ...
    def open_specific_file(self, file_path: str):
        ...
        try:
            effective_limit = getattr(self.main_window, 'limit', 99999)
        except Exception:
            effective_limit = 99999
        self.load_file_with_directory_thumbnails(file_path, limit=effective_limit, external_load=True)
