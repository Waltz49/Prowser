...
class ShortcutsSidebar(QWidget):
    ...
    def _on_settings_changed(self, new_settings: dict):
        ...
        if not new_settings:
            return
        relevant_keys = ('favorite_directories', 'move_destinations', 'move_keys_mode')
    ...
    def refresh_shortcuts(self):
        ...
        fav_rows.append(row(f"<b>↘<span style='font-size:20px;'>&nbsp;</span> <span style=color:{heading}>Keys for moving files:</span></b>&nbsp;&nbsp;{move_gear}"))
        ...
        # Click Mode label and combo on same line (after "Keys for moving files" heading)
        self.content_layout.addWidget(self._move_mode_row)
        self.content_layout.addWidget(self._move_mode_note_label)
        ...
        for i, dest in enumerate(destinations):
            ...
            if display and os.path.exists(dest):
                ...
                dest_rows.append(row(
                    f"{cmd_symbol} {i + 1} / ⌥{cmd_symbol} {i + 1}: &nbsp;&nbsp;{link}",
                    value_row=True,
                    compress_vertical_padding=True,
                ))
        if not any(dest and os.path.exists(dest) for dest in destinations):
            dest_rows.append(row(f'<span style="color:{_muted};">No valid move destinations are defined.</span>', value_row=True, compress_vertical_padding=True))
        last_drop = None
        if hasattr(self.main_window, 'file_tree_handler') and self.main_window.file_tree_handler:
            ft = getattr(self.main_window.file_tree_handler, 'file_tree', None)
            if ft and hasattr(ft, 'get_last_drop_location'):
        ...
        if last_drop and os.path.exists(last_drop):
            ...
            if use_links:
                ...
                dest_rows.append(row(f"⌘ 0 / ⌥⌘ 0: &nbsp;&nbsp;{link}", value_row=True, compress_vertical_padding=True))
            else:
            ...
            dest_rows.append(row(
                f'⌘ 0 / ⌥⌘ 0: &nbsp;&nbsp;<span style="color:{_muted};">Not yet set</span>',
                value_row=True,
                compress_vertical_padding=True,
            ))
