...
MACOS_SPACE_MODE_AVAILABLE = True
...
from PySide6.QtWidgets import (
    QApplication, QGestureEvent, QInputDialog, QLabel, QLineEdit, QMainWindow,
    QMessageBox, QPlainTextEdit, QProgressBar, QProgressDialog, QSplitter,
    QStackedWidget, QStatusBar, QStyle, QTextEdit, QVBoxLayout, QWidget
)
...
# Local
from browser_window.dialogs.about_dialog import AboutDialog
from workers.beachball_fix import safe_refresh_wrapper, safe_thumbnail_wrapper
from files.browse_view_handler import BrowseViewHandler
from thumbnails.combined_sidebar_widget import CombinedSidebarWidget
from config import (
    ImageBrowserConfig,
...
from browser_window.dialogs.help_api import APIDocumentationDialog
...
from browser_window.managers.image_display_manager import ImageDisplayManager
from keyboard_handler import KeyboardHandlerManager
from browser_window.managers.lock_manager import LockManager
from browser_window.managers.exif_operations_manager import ExifOperationsManager
...
from slideshow.slideshow2_manager import Slideshow2Manager
from slideshow.slideshow3_manager import Slideshow3Manager
...
from browser_window.managers.status_notification import StatusNotification
from thumbnails.thumbnail_canvas import create_message_pixmap
...
from browser_window.managers.thumbnail_display_manager import ThumbnailDisplayManager
from thumbnails.thumbnail_operations_manager import ThumbnailOperationsManager
from browser_window.managers.thumbnail_context_menu import ThumbnailContextMenuHandler
...
from thumbnails.view_manager import ViewManager
from browser_window.managers.view_mode_manager import ViewModeManager
from browser_window.managers.wallpaper_manager import WallpaperManager
from workers.window_background_workers import (
...
class ImageBrowserWindow(QMainWindow):
    ...
    def __init__(self, fullscreen: bool = False, 
        ...
        # Note: immediate_macos_space_mode handling moved to showEvent to ensure window is visible first
    ...
    def current_sort_mode(self, mode: SortMode):
        ...
        if (mode != SortMode.DUPLICATES and
            ...
            self.duplicate_sections = []
        if mode != SortMode.DUPLICATES and getattr(self, 'convert_conflict_context', None):
            self.convert_conflict_context = None
            if hasattr(self, 'update_convert_conflict_auto_rename_button'):
                self.update_convert_conflict_auto_rename_button()
    ...
    def set_reference_graph_presentation(
        ...
        if active and paths and len(paths) > 1:
            from search.reference_graph import build_reference_graph
    ...
    def refresh_from_configuration(self, configuration: dict, from_api: bool = False):
        ...
        Args:
            configuration: Dictionary containing files, directory, filter, etc.
        ...
        try:
            ...
            if fullscreen is True:
                # Explicitly set to True (e.g., --fullscreen flag): use macOS Space mode
                requested_macos_space_mode = True
            elif fullscreen is False:
                # Explicitly set to False (e.g., --no-fullscreen flag): windowed mode
                requested_macos_space_mode = False
            ...
            if requested_macos_space_mode and not self.isFullScreen():
                ...
                QTimer.singleShot(200, enter_macos_space_mode_deferred)
            ...
            if files:
                ...
                if valid_files:
                    if from_api:
                        ...
                        self.load_specific_files(
                            valid_files,
                            external_load=True,
                            force_specific_files_grid=force_specific_files_grid,
    ...
    def initialize_components(self):
        ...
        from browser_window.managers.rename_status_manager import RenameStatusManager
        ...
        from workers.idle_detector import IdleDetector
        from browser_window.managers.background_clip_controller import BackgroundClipController
        from cache.background_cache_importer import BackgroundCacheImporter
    ...
    def setup_ui(self):
        ...
        if getattr(self, "sidebar_chat_widget", None) is not None:
            self.combined_sidebar.set_chat_widget(self.sidebar_chat_widget)
        ...
        if getattr(self, "sidebar_jobs_widget", None) is not None:
        ...
        self.combined_sidebar.chat_visibility_changed.disconnect()
        ...
        def set_initial_sizes():
            ...
            # Defer tree init so window can show before QFileSystemModel + filter walks run
            # (filterAcceptsRow can recursively scan directories on the main thread).
            if self.file_tree_visible:
                QTimer.singleShot(0, self.ensure_tree_initialized)
            ...
            # Defer macOS menu shortcut priming until layout has settled (main thread timer).
            if hasattr(self, 'menu_manager'):
                self.menu_manager.schedule_startup_menu_shortcut_priming()
    ...
    # | macos_space_mode   | T           | T           | T           | (cmd-ctrl-f)                            |
    ...
    def _schedule_post_settings_menu_refresh(self):
        ...
        Connected to SettingsDialog.accepted; on_settings_changed schedules when favorites/move change.
        """
    ...
    def _any_left_sidebar_pane_visible(self) -> bool:
        if self.file_tree_visible or self.preview_visible or getattr(self, "chat_visible", False):
            return True
        if getattr(self, "combined_sidebar", None):
            return (
                ...
                or (
                    self.combined_sidebar.is_chat_visible()
                    if hasattr(self.combined_sidebar, "is_chat_visible")
                    else False
                )
            )
        return False
    ...
    def _on_tree_visibility_changed(self, visible):
        ...
        if visible or self.combined_sidebar.is_preview_visible() or self.combined_sidebar.is_chat_visible():
            left_width = self.sidebar_width
            main_width = total_width - left_width - right_width
            self._set_splitter_sizes_safe([left_width, main_width, right_width])
        else:
    ...
    def _on_preview_visibility_changed(self, visible):
        ...
        if visible or self.combined_sidebar.is_tree_visible() or self.combined_sidebar.is_chat_visible():
    ...
    def refresh_thumbnail_theme_styles(self):
        ...
        container = getattr(self, "thumbnail_container", None)
        if container is None:
            return
        canvas = getattr(container, "canvas", None)
        if canvas and canvas._is_reference_graph_mode():
            canvas._calculate_reference_graph_layout()
        if hasattr(container, "refresh_theme_styles"):
    ...
    def load_specific_files(
        ...
        return self.directory_loader.load_specific_files(
            file_paths, external_load, force_specific_files_grid, skip_filter_pattern
        )
    ...
    def load_file_with_directory_thumbnails(
        ...
        if not should_preserve_window_focus(self):
            self.activateWindow()
            self.raise_()
    ...
    def cache_faces(self, recursive: bool = True, max_depth: Optional[int] = None, after_scan=None, paths_to_scan: Optional[list] = None, directory_override: Optional[str] = None):
        ...
        from faces.face_scan_runner import get_image_list, run_scan
        from faces.face_cache import normalize_path_for_face_cache, has_cached_faces
        ...
        from faces.face_gathering_coordinator import set_foreground_face_scan_active
    ...
    def filter_by_person(
        ...
        from faces.known_faces_manager import get_subject
    ...
    def _start_throttled_thumbnail_loading(self, images_to_process):
        """Queue thumbnail loads via BackgroundImageLoader (visible-first)."""
        if not images_to_process:
            return
        if self.current_view_mode == 'thumbnail' and getattr(self, 'thumbnail_container', None):
            self._restart_canvas_thumbnail_loading_for_visible()
            return
        size = self.current_thumbnail_size
        from thumbnails.thumbnail_constants import THUMBNAIL_QUEUE_BATCH_SIZE
        batch_limit = THUMBNAIL_QUEUE_BATCH_SIZE * 4
        for i, path in enumerate(images_to_process):
            if i >= batch_limit:
                break
            self.cache_manager.get_thumbnail_async(path, size, priority=0)
    ...
    def start_background_thumbnail_loading_if_needed(self):
        ...
        # Check if thumbnails actually need loading before queueing requests
        if getattr(self, 'thumbnail_container', None):
            if hasattr(self.thumbnail_container, 'canvas') and self.thumbnail_container.canvas:
                ...
                with QMutexLocker(canvas.mutex):
                    ...
                    if not hasattr(canvas, 'thumbnails') or not canvas.thumbnails or len(canvas.thumbnails) == 0:
                        ...
                        # Only skip loading if all thumbnails already have pixmaps
                        if not needs_loading:
                            return
        
        # Start loading thumbnails - queue via BackgroundImageLoader
    ...
    def refresh_open_imagegen_dim_limits(self) -> None:
        """Re-clamp generation width/height after app max dimension setting changes."""
        from PySide6.QtWidgets import QApplication

        for widget in QApplication.topLevelWidgets():
            refresh = getattr(widget, "refresh_generation_dim_limits", None)
            if callable(refresh):
                try:
                    refresh()
                except Exception:
    ...
    def show_about(self):
        ...
        present_auxiliary_dialog(self.about_dialog)
    ...
    def show_why_written(self):
        ...
        present_auxiliary_dialog(self.why_written_dialog)
    ...
    def show_api_help(self):
        ...
        present_auxiliary_dialog(self.api_documentation_dialog)
    ...
    def show_command_line_help(self):
        ...
        present_auxiliary_dialog(self.command_line_help_dialog)
    ...
    def show_pf_help(self):
        ...
        present_auxiliary_dialog(self.pf_keys_help_dialog)
    ...
    def on_settings_changed(self, new_settings):
        ...
        if (
            ...
            tid = new_settings.get('ui_theme')
            if not tid:
                tid = self.config.load_settings().get('ui_theme')
            if not tid:
            ...
            if hasattr(self, 'file_tree_handler'):
                ...
                if self.file_tree_handler.is_tree_initialized():
                    if hasattr(self.file_tree_handler, 'filter_proxy') and self.file_tree_handler.filter_proxy:
                        proxy = self.file_tree_handler.filter_proxy
                        proxy._clear_image_caches()
                        proxy.refresh_filter_settings()
            ...
            if hasattr(self, 'file_tree_handler'):
                if self.file_tree_handler.is_tree_initialized():
                    if hasattr(self.file_tree_handler, 'filter_proxy') and self.file_tree_handler.filter_proxy:
                        proxy = self.file_tree_handler.filter_proxy
                        proxy._clear_image_caches()
                        proxy._refresh_follow_symlinks_cache()
                        proxy.refresh_filter_settings()
                        proxy.invalidateFilter()
                        proxy.layoutChanged.emit()
    ...
    def copy_file_path_to_clipboard(self):
        ...
        if getattr(self, 'current_view_mode', None) == 'thumbnail':
            ...
            if len(selected_files) > 1:
                ...
                copy_text_to_clipboard(copy_data, main_window=self)
                return
    ...
    def copy_user_comment_to_clipboard(self):
        ...
        from exif.exif_utils import get_usercomment_from_path, decode_usercomment
        ...
        copy_text_to_clipboard(text, main_window=self)
    ...
    def enter_macos_space_mode(self):
        """Show the app in a macOS Space (native fullscreen)."""
        self.browse_view_handler.enter_macos_space_mode()

    def enter_windowed_mode(self):
        """Show the app in windowed mode (exit macOS Space)."""
        self.browse_view_handler.enter_windowed_mode()

    def toggle_macos_display_mode(self):
        """Toggle between macOS Space and windowed display mode."""
        in_macos_space = is_macos_space_mode() or self.isFullScreen()
        if in_macos_space:
    ...
    def open_map_for_current_image(self):
        ...
        from files.map_manager import open_map_for_image, open_map_for_images
        from config import get_config
    ...
    def reset_date_to_exif(self):
        self.exif_operations_manager.reset_date_to_exif()

    def reset_exif_to_file_date(self):
        self.exif_operations_manager.reset_exif_to_file_date()
    ...
    def convert_selected_images(self):
        ...
        from files.convert_format import convert_selected_images
        convert_selected_images(self, selected_files)

    def update_convert_conflict_auto_rename_button(self):
        """Show/hide and enable the Auto Rename overlay on the thumbnail canvas."""
        container = getattr(self, "thumbnail_container", None)
        if not container or not hasattr(container, "update_convert_conflict_auto_rename_button"):
            return
        container.update_convert_conflict_auto_rename_button()

    def run_convert_conflict_auto_rename(self):
        """Dry-run: CNN similarity rename plan for conflicted source files."""
        from PySide6.QtWidgets import QApplication

        from browser_window.dialogs.convert_conflict_rename_plan_dialog import (
            ConvertConflictRenamePlanDialog,
        )
        from files.convert_conflict_auto_rename import (
            build_convert_conflict_rename_plan,
            get_conflict_source_paths,
        )
        from sort_mode import SortMode
        from utils import (
        ...
        context = getattr(self, "convert_conflict_context", None)
        if not context:
            return

        if getattr(self, "current_sort_mode", None) != SortMode.DUPLICATES:
            return

        if context.get("has_name_conflicts"):
            show_styled_warning(
                self,
                "Auto Rename Unavailable",
                "Auto Rename cannot resolve name conflicts where multiple sources "
                "would create the same output filename.\n\n"
                "Resolve those conflicts manually first.",
            )
            return
        ...
        if not conflict_sources:
            show_styled_information(
                self, "Suggest Renames", "No conflicted source files to rename."
            )
            return

        progress = create_file_operation_progress_dialog(
            self, "Calculating rename suggestions", len(conflict_sources)
        )

        def on_progress(done, total):
            if total:
                progress.setLabelText(f"Scanning {min(done + 1, total)} of {total}")
                progress.setValue(done)
            QApplication.processEvents()

        try:
            plan, unresolved = build_convert_conflict_rename_plan(
                self, context, progress_callback=on_progress
            )
        finally:
            progress.close()
        ...
        if not plan and not unresolved:
            show_styled_information(
                self,
                "Suggest Renames",
                "No rename proposals could be generated.",
            )
            return

        ConvertConflictRenamePlanDialog.show_confirmation(
            plan,
            unresolved,
            self,
            main_window=self,
    ...
    def resize_images(self):
        ...
        from browser_window.managers.resize_images import resize_selected_images
    ...
    def _interrupt_thumbnail_loading(self):
        """Interrupt ongoing thumbnail loading before directory load."""
        ...
        try:
            with QMutexLocker(background_loader.queue_mutex):
                background_loader.load_queue[:] = [
                    item for item in background_loader.load_queue
                    if not (
                        len(item) >= 3
                        and len(item[2]) >= 5
                        and item[2][0] == 'thumbnail'
                        and item[2][3] < min_priority
                    )
                ]
                heapq.heapify(background_loader.load_queue)
        except Exception:
            pass
    ...
    def _apply_transparency_color_to_pixmap(self, pixmap):
        ...
        if use_diamonds:
            ...
            from files.browse_view_handler import _draw_diamond_pattern
    ...
    def _update_chrome_menu_actions(self):
        ...
        if hasattr(self, 'toggle_chat_action') and hasattr(self, 'combined_sidebar'):
            ...
            self.toggle_chat_action.setChecked(chat_vis)
            self.toggle_chat_action.setText(
                'Hide Chat' if chat_vis else 'Show Chat'
            )
        if hasattr(self, 'toggle_chrome_action'):
    ...
    def changeEvent(self, event):
        ...
        if event.type() == QEvent.WindowStateChange:
            # Update macOS display mode menu choices
            self.menu_manager.update_macos_display_mode_menu()
