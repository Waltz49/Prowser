...
# Root-level Python files that are dev-only or generated by build scripts — not shipped in source copies.
ROOT_PY_EXCLUDE=(
    block_test.py
    defsize.py
    fast_test.py
...
echo "  5. ./pyInstallerBuild.sh --min   (minimal bundle: browse + CLIP + faces)"
echo
