...
class ThumbnailDisplayManager:
    ...
    def generate_thumbnails(self, force_refresh=False):
        ...
        if len(images_to_process) > self.main_window.limit:
            if self.main_window.is_reversed:
                images_to_process = images_to_process[-self.main_window.limit:]
            else:
                images_to_process = images_to_process[:self.main_window.limit]

        # Set the displayed images list
        self.main_window.displayed_images = images_to_process.copy()
    ...
    def _start_throttled_thumbnail_loading(self, images_to_process):
        ...
        from window_background_workers import ThumbnailLoadingWorker
