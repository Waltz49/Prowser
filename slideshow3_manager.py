...
# Target average number of visible frames (deprecated - now uses max_images setting)
...
class Slideshow3Manager:
    ...
    def update_slideshow3_settings(self, new_settings):
        ...
        # Also handle legacy max_images setting for compatibility
        if 'max_images' in new_settings:
            max_images_setting = new_settings['max_images']
            if max_images_setting != 0:  # 0 means unlimited, don't use it
                self.max_simultaneous_frames = max_images_setting
                self.config.update_setting('slideshow3_max_simultaneous_frames', self.max_simultaneous_frames)
                self._enforce_max_frames()
